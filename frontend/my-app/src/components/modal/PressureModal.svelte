<script>
	...
	export let pressure_now = null;
	export let temp_diff; 
	...
	function sendData() {
		...
		dispatch('pressure_modal_msg', data);
	}
    ...
    $: if (isMounted) {
		chart_pressure_2.data.labels = data.labels;
		chart_pressure_2.data.datasets = data.datasets;
		chart_pressure_2.update();
	}

	onMount(() => {
		const canvas_5 = document.getElementById('myPaChartModal');
		chart_pressure_2 = new Chart(canvas_5, {
			type: 'line',
			data: data,
			options: options,
			fill: '#990f02'
			...
			<div class="flex items-center">
				...
				<div class="ml-5 w-0 flex-1">
					...
					<dd class="flex items-baseline">
						...
						<div class="ml-2 flex items-baseline text-sm font-mono font-semibold text-red-500">
							<svg
							...
							<span class="sr-only"> Remained stable </span>
							{temp_diff} Pa
						</div>
					</dd>
				</div>
