<script>
	...
	export let temperature_now = null;
	...
	$: if (isMounted) {
		chart_temp_2.data.labels = data.labels;
		chart_temp_2.data.datasets = data.datasets;
		chart_temp_2.update();
	}
	...
	onMount(() => {
		const canvas_4 = document.getElementById('myChartTempModal');
		chart_temp_2 = new Chart(canvas_4, {
			type: 'line',
			data: data,
			options: options,
			fill: '#990f02'
