    ...
    if (receivedString == "handshake") {
        ...
        if (state == 1) {
            ...
            if (commandKey == "sensor") {
                if (commandValue == "all") {
                    //change the function depending on which microbit
                    getBabyData();
                    // getEnvironmentData();
                } else if (commandValue == "temp") {
                    randomWait()
                    radio.sendString("" + control.deviceName() + "=" + input.temperature())
                } else if (commandValue == "light") {
                    randomWait()
                    // process light level as a percentage out of 255 e.g. 80 = 80%
                    radio.sendString("" + control.deviceName() + "=" + Math.floor(input.lightLevel() / 255 * 100))
                }
            }
        }
    }
